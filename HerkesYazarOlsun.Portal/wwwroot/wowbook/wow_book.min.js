...
c||(c=this.opts.flipSoundFile);for(var b=[],e=0,f=c.length;e<f;e++)b.push('<source src="'+a+c[e]+'">');return d("<audio preload>"+b.join("")+"</audio>")[0]},_untouch:function(a){return a.originalEvent.touches&&a.originalEvent.touches[0]||a},touchSupport:function(){var a=this;a.elem.bind("touchstart.wowbook",function(c){if(d(c.target).closest("#guncelle").length)return;var b=c.originalEvent.touches;1<b.length||(a._touchStarted={x:b[0].pageX,y:b[0].pageY,timestamp:c.originalEvent.timeStamp,inHandle:d(c.target).hasClass("wowbook-handle")},a._touchStarted.inHandle&&
a.pageEdgeDragStart(a._untouch(c)))});document.addEventListener("touchmove",function(c){if(d(c.target).closest("#guncelle").length)return;if(a._touchStarted){var e=c.originalEvent||c;var b=e.touches;a._touchEnded={x:b[0].pageX,y:b[0].pageY,timestamp:e.timeStamp};if(a._touchStarted.inHandle)return a.pageEdgeDrag(a._untouch(c));20<Math.abs(a._touchEnded.x-a._touchStarted.x)&&c.preventDefault();c.preventDefault()}},{passive:!1});d(document).on("touchend.wowbook touchcancel.wowbook",function(c){if(d(c.target).closest("#guncelle").length)return;if(a._touchStarted){!a._touchEnded&&d(c.target).hasClass("wowbook-handle")&&
